...
class JsonFileCRUD {
  ...
  create(item, callback) {
    ...
    if (!item || typeof item !== 'object' || Array.isArray(item)) {
      return callback(new Error('Item must be an object'));
    }
    ...
    this.readAll((err, items) => {
      ...
      if (existingItem) {
        this.isWriting = false;
        this.processWriteQueue();
        return callback(new Error(`Item with ${this.idField} ${item[this.idField]} already exists`));
      }
  ...
  update(id, data, callback) {
    ...
    if (data[this.idField] && data[this.idField] !== id) {
      return callback(new Error(`Cannot change ${this.idField} field`));
    }
  ...
  processWriteQueue() {
    if (this.writeQueue.length > 0 && !this.isWriting) {
      ...
      } else if (queuedOperation.type === "delete") {
        // Handle delete operation
        this.delete(queuedOperation.id, queuedOperation.callback);
      } else {
